...
def processing_data(data_df):
    ...
    #在python3之后，map转换之后没办法直接变成list，而是需要外加list进行再转换
    data_df['alcohol'] = list(map(fix_right_alcohol,data_df['alcohol']))
    #print(type(map(fix_right_alcohol, data_df['alcohol'])))
    data_df['alcohol'] = map(fix_right_alcohol, data_df['alcohol'])
    ...
    #print(type(list(map(fix_weight,data_df['weight']))))
    data_df['weight'] = list(map(fix_weight,data_df['weight']))
    #print(data_df['weight'].unique())
    final_data = data_df[data_df['mins_brushing'] < 5]
    del final_data['Unnamed: 0']
    final_data.to_csv(os.path.join(output_path,'data_clean_1.csv'))
